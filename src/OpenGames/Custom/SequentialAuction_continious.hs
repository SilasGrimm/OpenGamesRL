...
module OpenGames.Custom.SequentialAuction_continious where
...
import Examples.Auctions.SequentialAuction (transformPayments)
...
import OpenGames.Custom.SimultaneousAuction_repeated_more_players (PlayerName, drawPlayerValuationByNature)
...
computePayoff playerInfo xs winnerFound = 
  ...
  in if playerHasMaxBid then (snd playerInfo - snd playerTuple, True) else (0.0, False)
...
actionSpace :: [Bid]
-- actionSpace = [0.1, 0.2..maxBid]
actionSpace = removeDuplicates (0.0 : [minimum valueSpace + (x - minimum valueSpace) / 2.0 | x <- valueSpace] ++ [minimum valueSpace + (x - minimum valueSpace) / 3.0 | x <- valueSpace])
  where
...
-- Fair ties are ensured by the shuffleBids component of the game itself (each winner has the same probbility to be first)
...
-- currently not using this, because we achieve randomness in the game itself, see getWinner
...
-- randomNumberInRange valueSpace = [opengame|
--     inputs    :    ;
--     feedback  :    ;
--     :----------------------------:
--     inputs    :    ;
...
-- shuffle bids and compute returns
--  shuffling the bids ensures that in the case of ties, the winner is sampled from the uniform distribution 
...
-- first stage of the sequential auction, this is equal to a FPSBA, except that we now have to get the winner and make sure he can not bid in the next stages
...
firstStage valSpace actionSpace = [opengame|
  ...
  outputs   :   (getWinner shuffledBids, [player1Value, player2Value, player3Value])  ;
  returns   :      ;
|]
...
printOptimalStrategy :: [(((PlayerValuation, Int, Int), Bid), Reward)] -> IO ()
printOptimalStrategy qList = do
    let bestActions = getBestActions qList
    
    putStrLn ""
    putStrLn "================================================================================"
    putStrLn "  OPTIMAL STRATEGY ANALYSIS"
    putStrLn "================================================================================"
    putStrLn "  Val   | Stage | Players |  Learned Bid  |  Theory (v/n) |  Diff   |  Reward "
    putStrLn "--------------------------------------------------------------------------------"
    
    -- Print every learned state sorted by Valuation then Stage
    mapM_ printRow bestActions
    
    putStrLn "================================================================================"

  where
    -- | Collapses the Q-Table list into a list of unique states with their best action
    getBestActions :: [(((PlayerValuation, Int, Int), Bid), Reward)] -> [((PlayerValuation, Int, Int), (Bid, Reward))]
    getBestActions xs = 
        let 
            -- Transform input list into (Key, Value) pairs for the Map
            -- Key: State, Value: (Bid, Reward)
            pairs = [ (state, (action, reward)) | ((state, action), reward) <- xs ]
            
            -- Comparison function: Keep the pair with the higher Reward.
            -- If rewards are equal, prefer the lower bid (optional preference)
            keepBest (bid1, r1) (bid2, r2)
                | r1 > r2   = (bid1, r1)
                | r2 > r1   = (bid2, r2)
                | otherwise = if bid1 < bid2 then (bid1, r1) else (bid2, r2)

            -- Create a Map to filter down to 1 action per state
            stateMap = Map.fromListWith keepBest pairs
        in 
            -- Convert back to list and sort by Valuation (asc), then Stage (asc)
            sortOn (\((v, _, s), _) -> (v, s)) (Map.toList stateMap)

    -- | Formats a single row
    printRow ((val, remaining, stage), (bid, reward)) = do
        let theoryBid = val / fromIntegral remaining
            diff = bid - theoryBid
            
        -- Only print if the Valuation is a "nice" number to keep output readable (Optional)
        -- Remove this 'if' block to see ALL 101 lines.
        if isTestPoint val then
            printf "  %.2f  |   %d   |    %d    |    %7.3f    |    %7.3f    | %+.3f | %7.4f\n" 
                   val stage remaining bid theoryBid diff reward
        else
            return ()

    -- | Helper to filter output for readability. 
    -- Returns True for 0.1, 0.2, 0.25, 0.33, 0.5, etc.
    isTestPoint :: Double -> Bool
    isTestPoint v = 
        let epsilon = 0.0001
            rem10 = (round (v * 10) :: Int)
            isRoundTenth = abs (v - (fromIntegral rem10 / 10.0)) < epsilon
            isQuarter = abs (v - 0.25) < epsilon || abs (v - 0.75) < epsilon
            isThird = abs (v - 0.33) < 0.01 || abs (v - 0.66) < 0.01
        in isRoundTenth || isQuarter || isThird || v == 1.0 || v == 0.0

checkQTable :: [(((PlayerValuation, Int, Int), Bid), Reward)] -> IO ()
checkQTable = printOptimalStrategy  
...
--   opponentVal1 <- getContinuousVal maxVal
--   opponentVal2 <- getContinuousVal maxVal
...
--                         then Nothing 
--                         else Just (playerVal', numberPlayers - k, k + 1)
...
optimalOpponentStrategy = Kleisli $ \((playerName, playerValuation), remainingBidders, roundIndex) -> 
  ...
  let
    ...
    bid = if playerValuation == 0.0 then 0 else minVal + (fromIntegral (numberPlayers - numberLots) / fromIntegral remainingBidders) * (playerValuation - minVal)
  in distFromList [(bid, 1.0)]

roundToActionSpace :: Bid -> Bid
roundToActionSpace b = snd $ minimum [(abs (b - a), a) | a <- actionSpace]
