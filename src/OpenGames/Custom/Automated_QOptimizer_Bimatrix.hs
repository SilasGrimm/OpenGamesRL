...
--   let agentStrat = Kleisli $ \_ -> distFromList [(chosenAction, 1.0)] -- choose sampled strat with probability 1
...
learningStep game q lens possibleActions opponentStrategy n = do
  ...
  let agentStrat = strategyFromLens q lens -- Kleisli $ \_ -> distFromList [(chosenAction, 1.0)] -- choose sampled strat with probability 1
      gameOptic = play game (agentStrat ::- opponentStrategy ::- Nil)
