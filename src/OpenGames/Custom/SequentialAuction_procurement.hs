...
module OpenGames.Custom.SequentialAuction_procurement where
...
--   in if playerHasMaxBid then snd playerInfo - snd playerTuple else 0.0
...
-- actionSpace = removeDuplicates ([x / 2.0 | x <- valueSpace] ++ [x / 3.0 | x <- valueSpace])
--   where
...
--   outputs   :   (getWinnerRandom [("Player1",player1Dec),("Player2",player2Dec), ("Player3",player3Dec)], [("Player1",snd player1Value),("Player2",snd player2Value), ("Player3",snd player3Value)])  ;
--   returns   :      ;
-- |]
...
--   outputs   :  getWinnerRandom [("Player1",player1Dec),("Player2",player2Dec), ("Player3",player3Dec)]  ;
--   returns   :      ;
-- |]
...
--   let bid = playerValuation * fromIntegral (numberPlayers - numberLots) / fromIntegral (remainingBidders)
